-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Create all automation-related tables (consolidated, idempotent)
--
-- This migration recreates tables that were defined in earlier migration files
-- but were never applied to the live database:
--
--   From 20260521100000_webhook_infrastructure.sql:
--     webhook_automations
--
--   From 20260521160000_automation_engine.sql:
--     automations
...
-- ── Prerequisite: set_updated_at trigger function ────────────────────────────
...
-- ── 2. automations (visual workflow engine) ───────────────────────────────────
...
create policy if not exists "automations_select_own" on public.automations
  for select using (auth.uid() = user_id);
create policy if not exists "automations_insert_own" on public.automations
  for insert with check (auth.uid() = user_id);
create policy if not exists "automations_update_own" on public.automations
  for update using (auth.uid() = user_id);
create policy if not exists "automations_delete_own" on public.automations
  for delete using (auth.uid() = user_id);
...
create policy if not exists "executions_select_own" on public.automation_executions
  for select using (auth.uid() = user_id);

-- Worker needs to INSERT/UPDATE executions (bypasses RLS via service role)

-- ── 4. automation_step_logs ───────────────────────────────────────────────────
...
create policy if not exists "logs_select_via_execution" on public.automation_step_logs
  for select using (
    exists (
...
create policy if not exists "scheduled_tasks_select_own" on public.scheduled_tasks
  for select using (auth.uid() = user_id);
...
create policy if not exists "ai_prompts_select_own" on public.ai_prompts
  for select using (auth.uid() = user_id);
create policy if not exists "ai_prompts_insert_own" on public.ai_prompts
  for insert with check (auth.uid() = user_id);
create policy if not exists "ai_prompts_update_own" on public.ai_prompts
  for update using (auth.uid() = user_id);
create policy if not exists "ai_prompts_delete_own" on public.ai_prompts
  for delete using (auth.uid() = user_id);
...
create policy if not exists "contact_scores_select_own" on public.contact_scores
  for select using (auth.uid() = user_id);
create policy if not exists "contact_scores_insert_own" on public.contact_scores
  for insert with check (auth.uid() = user_id);
create policy if not exists "contact_scores_update_own" on public.contact_scores
  for update using (auth.uid() = user_id);
...
create policy if not exists "segments_select_own" on public.contact_segments
  for select using (auth.uid() = user_id);
create policy if not exists "segments_insert_own" on public.contact_segments
  for insert with check (auth.uid() = user_id);
create policy if not exists "segments_update_own" on public.contact_segments
  for update using (auth.uid() = user_id);
create policy if not exists "segments_delete_own" on public.contact_segments
  for delete using (auth.uid() = user_id);

-- ── 9. contact_segment_members ───────────────────────────────────────────────
...
create policy if not exists "segment_members_select_own" on public.contact_segment_members
  for select using (
    exists (
